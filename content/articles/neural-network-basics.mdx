...
- If the sum of stimulating signals is less than the Threshold, the outpur is `0`
...
If we have one or two stimulating (=`true` or `1`) inputs, the sum is bigger or equal to the Threshold, and the output is `1`.
...
>
> **Either** input x₁ **OR** input x₂ is `1`.
...
>
> Now we'll give the NN one data entry, and it will will generate probabilities for all possbile solutions.
...
> (this all happens automatically, i.e. *we* don't really *tell* the NN if it's wrong)
>
