...
- Backpropagation
- Coding Example + Further Reading
...
Such a NN could have an accuracy of nearly 100% on the data it was trained with, 
but it will fail recognizing anything that was not part of the Training Data.
(That's definitly not what we want)
...
> Fun Fact:
>
> A group of students once tried autonomous driving, and recorded hundrets of hours of driving.
> The AI drove quite good, but after a while the car reached a bridge, where the car suddenly stopped.
>
...
and thousands of customers want to know why their car suddenly stopped (best case, could have crased too).
...
They are all only 28 x 28 pixel small, i.e. the training speed is bigger, and you don't need a graphic card.
...
they are already seperated into training and test data and so on... The perfect dataset.

Additionally is quite easy to load the data, so we won't need much code for this.

The MNIST dataset is kinda the "Hello World" exmaple for Deep Learning.
