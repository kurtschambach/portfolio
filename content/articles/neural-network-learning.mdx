...
vector = [
    0.2,
    0.15,
    3.14, # we need to make the corresponding variable much bigger
    -0.5, # and this a little bit smaller
    0.1,
]
```
...
with `w`/`w'` being the variable / new variable.


### Problems

There are a few important things concerning the gradient descent.  
One of them is a good learning rate.
...
Another Problem are lokal minima.
As we want to find the absolute minimum of the errorfunction we need the global minimum.

When we hav some randow startting point, it can happen, that the way of quickest decrease, 
i.e. gradient decent leads us to a local minima instaed of the global one.
...
because in a local minima you don't have any way decrease near you current point,
...
If not, just try it another time, and the result should be better.

If not, it's probably your fault :/

---

Some of you may be asking now: 
"why is something changing when we start in once again?"  
With this question we come to the topic of initialisation.

An optimal point for the initialisation would be great, 
i.e. in our three-dimensional example a point that is already near to a (global) minimum.
...
To classify a NN we usually take the **accuracy**, and additionally the **loss**.
Accuracy is quite self-explainatory:

`accuracy = correct classifications / all classifications`

TODO: loss explanation

For more information we can use small parts of the test data while training, 
