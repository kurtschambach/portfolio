...
>
> **Binary classification** is for example separating images of dogs and cats, 
> i.e. there are always two labels.  
> **Multiclass classification** would be assigning images one of three or more labels,
> e.g. for images of animals assigning them on of the labels dog, cat, octopus, etc.

Supervised learning is basically what we've been talking about previously regarding training.
...
When training our NN it needs some kind of feedback 
to know which variables (thresholds, weights, etc.) it has to improve. 
...
For this we will use an **error function**, also called *cost function* (or *loss function*).
...
`w' = w - n * gradient`  
with `w`/`w'` being the variable / new variable ans `n` the learning rate, 
...
I've already mentioned the loss somewhere, but for a clear definition:  
